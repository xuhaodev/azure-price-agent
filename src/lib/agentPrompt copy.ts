...
2. **ONLY use eq for exact fields: armRegionName**
...
3. **Progressive fuzzy broadening** if no results (max 3 attempts):
   ...
   - AI Models: contains(armSkuName, 'GPT 5 Mini') → contains(armSkuName, 'GPT 5') → contains(armSkuName, 'GPT')
...
4. **Handle ambiguous naming with OR**:
   ...
   - AI: (contains(serviceName, 'GPT') or contains(productName, 'GPT5'))

5. **Complete query examples** (note: ALL names use contains, ONLY region/priceType use eq):
   - VM: armRegionName eq 'eastus' and contains(serviceName, 'Virtual Machines') and contains(armSkuName, 'D8s_v4')'
   - Redis: armRegionName eq 'westus2' and (contains(serviceName, 'Redis') or contains(productName, 'Cache')) and contains(armSkuName, 'M50') '
   - SQL: armRegionName eq 'centralus' and contains(serviceName, 'SQL Database') and contains(armSkuName, 'S3') '
   - Storage: armRegionName eq 'westeurope' and contains(serviceName, 'Storage') and contains(meterName, 'LRS') '
